...
进度更新：2026-06-02 已完成阶段 B 后台角色细分，并推进阶段 A 的 `admin_preferences` 与 `ops_triage` 表级 repository。当前已新增 `operator` 运营只读角色，可查看独立后台运营数据并保存个人筛选偏好，组织、成员、接口、账单、反馈和错误跟进等写操作仍限定 owner/admin。
...
状态：进行中，`admin_preferences` 与 `ops_triage` 已完成表级读写，执笔人相关表待继续。
...
- PostgreSQL Store 下 `admin_preferences` 和 `ops_triage` 可独立读写。
- JSON Store 行为不变。
- 不引入跨表快照覆盖风险。
