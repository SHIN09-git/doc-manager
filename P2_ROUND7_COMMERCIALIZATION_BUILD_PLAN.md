...
进度更新：2026-06-02 已完成阶段 B 后台角色细分，并推进阶段 A 的 `admin_preferences` 表级 repository。当前已新增 `operator` 运营只读角色，可查看独立后台运营数据并保存个人筛选偏好，组织、成员、接口、账单、反馈和错误跟进等写操作仍限定 owner/admin。
...
状态：进行中，`admin_preferences` 已完成表级读写，`ops_triage` 和执笔人相关表待继续。

任务：

- [x] 为 `admin_preferences` 增加 repository，并接入后台偏好读取、保存和清空。
- [ ] 为 `ops_triage` 增加 repository。
...
- PostgreSQL Store 下 `admin_preferences` 可独立读写，`ops_triage` 待继续。
- JSON Store 行为不变。
- 不引入跨表快照覆盖风险。
