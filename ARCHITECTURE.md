...
| `src/modules/documents/` | 文档导入、阅读、导出、侧栏控制、垃圾箱、真实文件夹关联 |
| `src/modules/editor/` | 正文编辑器周边控制器，例如右键菜单、键盘导航和局部编辑动作 |
...
后续重构建议继续把 `app.js` 中的页面控制逻辑迁移到更小的控制器模块。当前已完成接口设置、编辑器右键菜单、文档侧栏、执笔人工作台和执笔人生成弹窗控制器拆分，下一批优先处理云端面板和编辑器查找替换等仍留在主入口的功能。
...
- 继续缩小 `app.js`，优先把云端面板、编辑器查找替换和自定义文档类型控制逻辑继续拆出；接口设置、文档侧栏、编辑器右键菜单、执笔人工作台和执笔人生成弹窗已拆为独立控制器。
- 给执笔人版本差异增加更清晰的可视化对比。
- 为导入执笔人包增加更完整的预览确认界面。
- 增加可选后端代理方案文档，便于团队部署。
