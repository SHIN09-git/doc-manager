...
- 错误：`system_events` 保存组织归属的系统事件；后台可直接更新 warn/error 事件 metadata 保存跟进状态；`ai_usage` 失败记录通过 `ops_triage` 保存跟进状态，避免污染原始用量事实；PostgreSQL Store 下系统错误和 AI 失败跟进均已走表级 repository 写入。
