...
后续重构建议继续把 `app.js` 中的页面控制逻辑迁移到更小的控制器模块。当前已完成接口设置、云端 API 客户端、云端会话控制、云端文档/执笔人同步控制、云端用户操作控制、云端面板渲染、编辑器查找替换、编辑器右键菜单、文档侧栏、自定义文档类型、执笔人工作台和执笔人生成弹窗控制器拆分，下一批优先处理功能地图动作、主视图路由和少量全局页面编排。
...
- 继续缩小 `app.js`，下一步优先拆出功能地图动作、主视图路由和少量全局页面编排；接口设置、云端 API 客户端、云端会话控制、云端文档/执笔人同步控制、云端用户操作控制、云端面板渲染、文档侧栏、自定义文档类型、编辑器查找替换、编辑器右键菜单、执笔人工作台和执笔人生成弹窗已拆为独立模块。
- 给执笔人版本差异增加更清晰的可视化对比。
- 为导入执笔人包增加更完整的预览确认界面。
- 增加可选后端代理方案文档，便于团队部署。
