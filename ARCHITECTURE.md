...
| `src/modules/cloud/` | 云端面板渲染、云端账单、人工充值和费用明细的前端展示格式化工具 |
...
后续重构建议继续把 `app.js` 中的页面控制逻辑迁移到更小的控制器模块。当前已完成接口设置、云端面板渲染、编辑器查找替换、编辑器右键菜单、文档侧栏、自定义文档类型、执笔人工作台和执笔人生成弹窗控制器拆分，下一批优先处理云端请求/同步编排等仍留在主入口的功能。
...
- 继续缩小 `app.js`，优先把云端请求/同步编排继续拆出；接口设置、云端面板渲染、文档侧栏、自定义文档类型、编辑器查找替换、编辑器右键菜单、执笔人工作台和执笔人生成弹窗已拆为独立模块。
- 给执笔人版本差异增加更清晰的可视化对比。
- 为导入执笔人包增加更完整的预览确认界面。
- 增加可选后端代理方案文档，便于团队部署。
