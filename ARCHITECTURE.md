...
| `src/modules/skills/` | 执笔人解析、聚合、生成弹窗、渲染、导入导出、版本和测试 |
...
后续重构建议继续把 `app.js` 中的页面控制逻辑迁移到更小的控制器模块。当前已完成接口设置、编辑器右键菜单和执笔人生成弹窗控制器拆分，下一批优先处理执笔人工作台和文档侧栏。
...
- 继续缩小 `app.js`，把执笔人工作台和文档管理控制器拆出；接口设置、编辑器右键菜单和执笔人生成弹窗已拆为独立控制器。
- 给执笔人版本差异增加更清晰的可视化对比。
- 为导入执笔人包增加更完整的预览确认界面。
- 增加可选后端代理方案文档，便于团队部署。
