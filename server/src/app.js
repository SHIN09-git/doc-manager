...
} from "./billing/manualPaymentService.js";
...
async function route(ctx) {
  ...
  if (request.method === "GET" && path === "/api/billing/manual-orders") return manualPaymentHandlers.listManualPaymentOrders(ctx);
  if (request.method === "POST" && path === "/api/billing/manual-orders") return manualPaymentHandlers.createManualPaymentOrder(ctx);
  if (request.method === "POST" && /^\/api\/billing\/manual-orders\/[^/]+\/review$/.test(path)) {
    return manualPaymentHandlers.reviewManualPaymentOrder(ctx, path.split("/")[4]);
  }
