...
面向域名部署、生产环境、人工充值、邮件、备份和灰度运营的剩余工作，统一看 [WEBSITE_LAUNCH_TODO.md](WEBSITE_LAUNCH_TODO.md)。

这份上线总控清单已经按“一页执行看板 → 当前上线判断 → 你需要准备的事情 → 我可以继续完成的事情 → 上线 TODO 总表 → 最短上线顺序”重整。资源侧包括域名、VPS、PostgreSQL、AI Key、邮件服务、微信/支付宝收款码、套餐规则、管理员和合规文本；代码侧后续施工、真实环境验收和灰度运营也按 P0/P1/P2 拆好。

## 2026-06-09 Store 写入失败回滚修复记录
...
- [x] 统一反馈处理响应公开字段，单条和批量反馈状态更新会返回脱敏后的系统事件对象。
- [x] 加固文档、文件夹与执笔人列表渲染属性边界，转义本地/导入数据中的文档 ID、文件夹 ID、执笔人 ID，并清洗文件夹颜色值。
- [x] 加固云端人工充值收款码与独立后台支付升级 URL 边界，危险协议或控制字符不会写入图片地址或被打开。
- [x] 加固导出和真实文件夹写入文件名清洗，控制字符、纯空白、尾随点号/空格和 Windows 保留设备名会被兜底处理。
- [x] 加固邮箱验证和密码重置提交阶段错误反馈，未知邮箱和错误令牌会返回统一错误，降低账号枚举风险。
- [x] 加固生产 Cookie 写请求来源门禁，生产环境中 Cookie 认证的写操作必须带可信 `Origin`/`Referer`，脚本集成可改用 Bearer。
- [x] 加固生产 JSON 环境变量解析，`AI_COST_RATES`、`PAYMENT_PLAN_PRICE_MAP` 和 `MANUAL_PAYMENT_PACKAGES` 填写后必须是合法 JSON 对象或数组，避免价格映射、充值套餐或成本配置静默失效。
- [x] 修复 AI 成本估算配置契约漂移：后端兼容 `prompt`/`completion` 每 token 单价别名，生产示例改用明确的 `prompt_per_1k`/`completion_per_1k`。
- [x] 加固部署自检中的人工充值套餐校验，复用后端套餐归一化逻辑，避免无效套餐数组通过真实网站上线检查。
- [x] 加固人工充值运行时套餐归一化，零金额、非数字金额和非数字额度的套餐不会进入用户可提交订单列表。
- [x] 限制人工充值套餐中显式填写的 `plan` 只能是 Pro/Team 付费档，避免误配置为付费购买免费档。
- [x] 加固正式支付 checkout 配置：`PAYMENT_CHECKOUT_MODE=webhook` 必须配置签名密钥和 pro/team 价格映射，缺少映射时不会展示或创建不可靠的支付链接。
- [x] 加固备份生成可靠性，`server:backup` 改为临时文件原子写入、重命名后回读校验，确认结构和表计数后再上传对象存储。
- [x] 加固管理后台用量和审计 CSV 导出，疑似公式内容会按文本导出，避免管理员打开表格时触发公式注入。
...
- [x] 修复云端账号删除后的成员关系残留问题，删除账号会同步移除该用户所有组织成员关系，并在审计记录中写入清理数量和组织 ID。
- [x] 修复注册时验证邮件投递失败导致账号已创建但接口返回失败的问题；现在注册仍返回成功会话，并提示用户稍后重新发送验证码。
- [x] 加强 GitHub Actions 收口覆盖，CI 现在会安装 Chromium 并运行 `npm run test:e2e`，避免端到端工作台流程只在本地验证。
- [x] 统一 IndexedDB 打开入口兜底，真实文件夹授权库在浏览器不支持 IndexedDB 时会返回清晰错误，而不是抛出底层运行时异常。
- [x] 修复本地存储降级状态误导问题：工作台会记录当前使用 IndexedDB 还是 localStorage 兜底，且兜底保存必须同时写入数据和启动标记才算成功。
- [x] 加固执笔人测试保存门禁：当 normal/missing_fact/leakage 测试用例已经失败但模型漏填风险计数时，本地归一化仍会派生硬规则漏命中、事实编造、个案复用或隐私泄漏风险并阻止正式保存。
...
- [x] 统一编辑器右键菜单复制逻辑：复制正文片段时复用 `copyTextToClipboard()`，降级路径不可用时提示手动复制并返回失败结果，不再静默误报。
- [x] 加固独立管理后台跳转：当浏览器环境缺少可写 `location` 或跳转被拦截时返回失败并提示，不再误报已进入后台。
- [x] 加固独立管理后台导出下载：抽出可测试下载工具，下载 API 不可用或对象 URL 创建失败时提示失败，不再误报导出成功。
- [x] 加固生产 live AI 配置门禁：`AI_PROXY_MODE=live` 必须配置平台 Key 或显式允许组织后台 Key，上线自检不再把缺少 Key 来源当作 warning 放行。
- [x] 加固云端 live AI 请求模型门禁：live 代理缺少模型名时返回 `missing_ai_model`，不再把 `"default"` 发给服务商，也不消耗额度或写入失败用量。
- [x] 加固生产 live AI 默认模型门禁：`AI_PROXY_MODE=live` 时生产启动校验和上线自检都要求配置 `AI_MODEL`，避免自检通过后首个真实 AI 请求才失败。
- [x] 加固 live AI 空响应处理：上游返回 200 但没有可用文本时记录为失败用量和系统事件，不再返回空文档或按成功请求处理。
...
下一步建议：继续拆 PostgreSQL 写入路径，并补真实支付渠道与邮件域名联调记录。

## 2026-05-24 下一阶段待施工重点

- [x] 收口 `/api/ops/recent-errors` 权限边界，组织管理员默认只能看本组织事件，不能看到平台级全局事件。
- [x] 统一 AI 失败记录与系统错误事件的跟进契约，避免“能展示但无法保存跟进”。
...
- [x] 管理汇总 API：新增 `GET /api/admin/dashboard`，owner/admin 可管理，operator 可只读查看。
