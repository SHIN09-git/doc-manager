...
后续结构优化建议：继续把 `app.js` 的云端面板控制逻辑拆成 `src/modules/cloud/cloudController.js`，再把 `server/src/app.js` 中的 billing route 编排拆为 `server/src/billing/manualPaymentService.js`。这两步会触碰更多调用点，建议单独施工。
...
- 最近一次完整验证：161 项前端与核心单元测试通过，54 项后端商业化 API 测试通过，30 项端到端测试通过。
